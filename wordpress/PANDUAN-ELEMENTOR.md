...
sesuai brand guideline resmi (warna, font, tombol, kartu) dengan nuansa
**ceria, ramah, dan premium**.

Isi folder `wordpress/`:
- `kurnia-brand.css` — gaya global (warna + font + tombol + form).
- `sections/` — 5 blok section siap tempel (Hero, Menu Favorit, Keunggulan, Testimoni, CTA/Reservasi).
...
| Slot | Font | Catatan |
|------|------|---------|
| Primary (judul) | **Alexandria** | Headline & sub-headline |
| Secondary (sub) | **Crimson Pro** | Sub-headline |
| Text (body) | **Crimson Pro** | Isi paragraf |
| Accent | **Alexandria** | Tombol/label |
...
`01-hero` → `02-menu-favorit` → `03-keunggulan` → `04-testimoni` → `05-cta-reservasi`.
...
- **Menu & harga** — nama hidangan, deskripsi, harga di `02-menu-favorit.html`.
- **Kontak** — nomor WhatsApp (`https://wa.me/62...`), telepon, alamat, jam
  buka di `05-cta-reservasi.html`.
- **Testimoni** — kutipan & nama pelanggan asli di `04-testimoni.html`.
